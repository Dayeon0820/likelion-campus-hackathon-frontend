...
const ClassList = () => {
    ...
    const fetchClassData = async (category, classType) => {
        try {
            const response = await fetch(`http://sangsang2.kr:8080/api/lecture/category?category=${category}`);
            const data = await response.json();

            const transformedData = data
                .filter((lecture) => classType === "allClass" || lecture.type === classType)
                .map((lecture) => ({
                    id: lecture.id,
                    title: lecture.name,
                    price: `₩ ${lecture.price.toLocaleString()}`,
                    image: lecture.imageUrl[0]?.imageUrl || "https://via.placeholder.com/100"
                }));
                ...
                <div className="headerCategories">
                    ...
                    <ul className="categoryList">
                        {Object.keys(categoryMap).map((list, i) => (
                            <li
                                key={i}
                                className={`${categoryMap[list] === selectedCategory ? "active" : ""} categoryTxt`}
